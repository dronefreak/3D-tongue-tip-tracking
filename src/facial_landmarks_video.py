#!/usr/bin/env python3
"""
2-D facial landmark detection and mouth-coordinate tracking from video.

Detects 68 facial landmarks in each video frame using dlib's HOG face
detector and a shape predictor model, then records the position of landmark
#48 (left mouth corner) as a proxy for tongue-tip location in a single
camera view.
...
-----
  ...
  # Faster batch processing (no live window, every 2nd frame)
  python src/facial_landmarks_video.py -p model.dat -v video.avi \\
      --no-display --skip-frames 2 --export-csv out.csv --export-json out.json

Notes
-----
  * This script has no ``if __name__ == "__main__":`` guard; all top-level
    code executes on import.  Wrap in a guard before importing in other
    modules.
  * Requires ``dlib`` and ``imutils`` (not part of the minimal dev install).
    Download the GTX shape predictor with::

        curl -L https://github.com/davisking/dlib-models/raw/master/ \\
             shape_predictor_68_face_landmarks_GTX.dat.bz2 | bzip2 -d \\
...
ap.add_argument(
    "--no-display", action="store_true", help="disable video display for faster batch processing"
)
ap.add_argument(
    "--skip-frames",
    type=int,
...
ap.add_argument("--output-video", type=str, help="save annotated video to file (e.g., output.avi)")
